...
exports.Project = class {
  ...
  getMissingDependencies(bundle, dependencies, ignore) {
    if (!dependencies) {
      return ;
    }
    let missingDependencies = Object.assign({}, dependencies);
    const bundleName = (bundle.name || bundle);
    for (let dep of Object.keys(dependencies)) {
      if (ignore && ignore.indexOf(dep) >= 0) {
        delete missingDependencies[dep];
        continue;
      }
      for (let b of this.model.build.bundles) {
        if (this.hasDependency(b, dep)) {
          delete missingDependencies[dep];
          break;
        }
      }
    ...
    return Object.keys(missingDependencies).map(function (value, index, arr) { return (this[value].indexOf('@') > 0 || this[value] === value ? this[value] : `${value}@${this[value]}`); }, missingDependencies);
  }
