...
module.exports = class {
  ...
  execute(context) {
    ...
    if (context.state.unitTestRunners) {
      model.unitTestRunners = context.state.unitTestRunners;
    }
    ...
    if (model.features.navigation) {
      model.features.bootstrap = 'bootstrap';
      model.features.fontAwesome = 'fontAwesome';
    }
  ...
  createProjectDescription(model) {
    ...
    if (model.unitTestRunners) {
      for (let i = 0; i < model.unitTestRunners.length; i++) {
        text += `    <magenta><bold>Unit Test Runner: </bold></magenta>${model.unitTestRunners[i].displayName}` + os.EOL;
      }
    }
    ...
    let features = Object.keys(model.features);
    text += `    <magenta><bold>Features: </bold></magenta>${features}` + os.EOL;
