  ...
  addFile(file, inclusion) {
    ...
    if (!found) {
      for (let sourceType of sourceTypes) {
        if (sourceType.applies(file.path)) {
          found = new sourceType(this, file, (inclusion ? inclusion.loaderConfig : undefined));
          break;
        }
      }
    ...
    return analyzeDependency(this.packageAnalyzer, dependency).then(description => {
      ...
      if (description.singleFile) {
        ...
        if (loaderConfig.main) {
          config.main = loaderConfig.main;
        }
        this.loaderConfig.packages.push(config);
      }
