...
exports.Bundle = class {
  ...
  getBundledFiles() {
    return this.includes.reduce((a, b) => a.concat(b.getBundledFiles()), []).filter(x => x);
  }

  getCopiedFiles() {
    return this.includes.reduce((a, b) => a.concat(b.getCopiedFiles()), []).filter(x => x);
  }
  ...
  write(platform) {
    ...
    work = work.then(() => copies = copies.concat(this.getCopiedFiles()));
    ...
    work.then(() => {
      for (let i = 0; i < copies.length; ++i) {
        let currentFile = copies[i];

        let output = path.join(platform.output, "..");
        let root = path.join(process.cwd());
        let src = path.normalize(currentFile.path);
        let prefix = path.resolve(process.cwd(), 'aurelia_project', currentFile.loaderConfig.path);
        let name = currentFile.loaderConfig.name;
        let dest = src.replace(prefix, path.join(output, name));
        ...
        let arr = dest.replace(root, '').split(/[\\\/]/);
        arr.pop();
        let folder = root;
        while (arr.length) {
          folder = path.join(folder, arr.shift());
