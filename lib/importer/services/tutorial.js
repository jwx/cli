...
const requiredResourcesExtensions = ['.css', '.html'];
...
module.exports = class {
    ...
    .then(tutorial => {
      ...
      if (metadata && metadata.dependencies) {
        let dependenciesWithResources =
            metadata.dependencies.filter(x => x.resources && this.requiredResources(x.resources).length > 0);
        ...
        if (dependenciesWithResources.length > 0) {
          ...
          for (let dep of dependenciesWithResources) {
            logger.info(`${dep.name} has resources. The following require statements can be used to load these resources:`);
              ...
              for (let resource of this.requiredResources(dep.resources)) {
  ...
  requiredResources(resources) {
    return resources.filter(x => requiredResourcesExtensions.indexOf(path.extname(x)) !== -1);
  }
