...
module.exports = class {
  ...
  import(packages, alreadyProcessed, packageInstaller) {
    logger.debug('ImportEngine importing:', packages, " skipping:", alreadyProcessed);
    let index = 0;
    let that = this;
    ...
    function _importPackage() {
      ...
      return that.importPackage(packages[index++], alreadyProcessed, packageInstaller)
      .then(_importPackage);
    }
  ...
  importPackage(pkg, alreadyProcessed, packageInstaller) {
    let container = this.getContainer(pkg);
    let importer = container.get(PackageImporter);
    return importer.import(this, alreadyProcessed, packageInstaller);
  }
  ...
  getContainer(pkg) {
    ...
    container.registerAlias(Tracer, 'tracer');

    pkg.resourceInclusion = container.get('resource-inclusion');
    pkg.tracer = container.get('tracer');
